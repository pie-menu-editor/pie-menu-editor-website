...
test("generated output is isolated, exact-origin, and deny-by-default", async () => {
  ...
  assert.match(application, /setupTitle\.textContent = "Save your Repository access information"/u);
  assert.match(application, /setupTitle\.textContent = "Setup complete"/u);
  assert.match(html, /The Repository access token and Recovery secret will not be shown again after setup/u);
  assert.match(html, /Have a password manager, or another secure place you can find years from now/u);
  assert.match(html, /save the Repository access token and Recovery secret\s+before finishing setup/u);
  assert.match(html, /Paste this into Blender's <strong>URL<\/strong> field/u);
  assert.match(html, /Paste this into Blender's <strong>Secret<\/strong> field/u);
  assert.match(html, /Save this outside Blender in a password manager/u);
  assert.match(html, /You will need it to extend updates\s+or replace Repository access/u);
  assert.match(html, /I saved the Repository token and Recovery secret/u);
  assert.match(html, /Open Blender Preferences and select <strong>Get Extensions<\/strong>/u);
  assert.match(html, /Open Installation Guide/u);
  assert.doesNotMatch(html, /Public address|Private|illustrated Installation Guide/u);
